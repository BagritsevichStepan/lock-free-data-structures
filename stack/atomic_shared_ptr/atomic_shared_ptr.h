#ifndef LOCK_FREE_DATA_STRUCTURES_ATOMIC_SHARED_PTR_H
#define LOCK_FREE_DATA_STRUCTURES_ATOMIC_SHARED_PTR_H
...
namespace LFStructs {
    ...
    inline constexpr int kMagicLen = 16;
    inline constexpr size_t kMagicMask = 0x0000'0000'0000'FFFF;

    template<typename T>
    struct alignas(concurrent::cache::kCacheLineSize) ControlBlock {
    ...
    template<typename T>
    class alignas(concurrent::cache::kCacheLineSize) FastSharedPtr {
    public:
        ...
        ControlBlock<T> *getControlBlock() { return reinterpret_cast<ControlBlock<T> *>(knownValue >> kMagicLen); }
    ...
    private:
        void destroy() {
            if (foreignPackedPtr != nullptr) {
                ...
                while (!foreignPackedPtr->compare_exchange_weak(expected, expected - 1)) {
                    if (((expected >> kMagicLen) != (knownValue >> kMagicLen)) || !(expected & kMagicMask)) {
                        ControlBlock<T> *block = reinterpret_cast<ControlBlock<T> *>(knownValue >> kMagicLen);
        ...
        FastSharedPtr(std::atomic<size_t> *packedPtr)
            ...
            int diff = knownValue & kMagicMask;
    ...
    template<typename T>
    class alignas(concurrent::cache::kCacheLineSize) AtomicSharedPtr {
    public:
    ...
    AtomicSharedPtr<T>::AtomicSharedPtr(T *data) {
        ...
        packedPtr.store(reinterpret_cast<size_t>(block) << kMagicLen);
    }
    ...
    SharedPtr<T> AtomicSharedPtr<T>::get() {
        ...
        auto block = reinterpret_cast<ControlBlock<T> *>(packedPtrCopy >> kMagicLen);
        ...
        while (true) {
            ...
            if (((expected >> kMagicLen) != (packedPtrCopy >> kMagicLen)) ||
                ((expected & kMagicMask) == 0)) // >20 hours wasted here
            {
    ...
    AtomicSharedPtr<T>::~AtomicSharedPtr() {
        ...
        auto block = reinterpret_cast<ControlBlock<T> *>(packedPtrCopy >> kMagicLen);
        size_t diff = packedPtrCopy & kMagicMask;
        if (diff != 0) {
            block->refCount.fetch_add(diff);
        }
    ...
    bool AtomicSharedPtr<T>::compareExchange(T *expected, SharedPtr<T> &&newOne) {
        ...
        if (holder.get() == expected) {
            ...
            size_t desiredPackedPtr = reinterpret_cast<size_t>(newOne.controlBlock) << kMagicLen;
            size_t expectedPackedPtr = holdedPtr << kMagicLen;
            while (holdedPtr == (expectedPackedPtr >> kMagicLen)) {
                if (expectedPackedPtr & kMagicMask) {
                    int diff = expectedPackedPtr & kMagicMask;
                    holder.getControlBlock()->refCount.fetch_add(diff);
                    if (!packedPtr.compare_exchange_weak(expectedPackedPtr, expectedPackedPtr & ~kMagicMask)) {
                        holder.getControlBlock()->refCount.fetch_sub(diff);
                    }
                    continue;
                }
                assert((expectedPackedPtr >> kMagicLen) != (desiredPackedPtr >> kMagicLen));
                if (packedPtr.compare_exchange_weak(expectedPackedPtr, desiredPackedPtr)) {
                    newOne.controlBlock = nullptr;
                    assert((expectedPackedPtr >> kMagicLen) == holdedPtr);
    ...
    void AtomicSharedPtr<T>::destroyOldControlBlock(size_t oldPackedPtr) {
        auto block = reinterpret_cast<ControlBlock<T> *>(oldPackedPtr >> kMagicLen);
...
#endif //LOCK_FREE_DATA_STRUCTURES_ATOMIC_SHARED_PTR_H
