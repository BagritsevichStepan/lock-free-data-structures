...
namespace concurrent::queue {
    ...
    private:
        ...
        alignas(cache::kCacheLineSize) std::atomic<size_t> tail_{0};
        size_t cached_head_{0};

        PADDING(padding2_, sizeof(std::atomic<size_t>) + sizeof(size_t));

        alignas(cache::kCacheLineSize) std::atomic<size_t> head_{0};
        size_t cached_tail_{0};

        PADDING(padding3_, sizeof(std::atomic<size_t>) + sizeof(size_t));
    };
    ...
    template<typename T, size_t Capacity>
    size_t BoundedSPSCQueue<T, Capacity>::GetSize() const noexcept {
    ...
    template<typename T, size_t Capacity>
    size_t BoundedSPSCQueue<T, Capacity>::GetCapacity() const noexcept {
        return GetBufferSize();
    }


    template<typename T, size_t Capacity>
    constexpr size_t BoundedSPSCQueue<T, Capacity>::GetBufferSize() {
    ...
    template<typename T, size_t Capacity>
    constexpr size_t BoundedSPSCQueue<T, Capacity>::GetIndexMask() {
